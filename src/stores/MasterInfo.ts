...
export const useMasterInfoStore = defineStore('masterInfoStore', () => {
  ...
  const getFreeTimeSlotsForDate = async (date: string) => {
    try {
      ...
      const res = await axios.get(
        `${API_URL_MASTER}/GetFreeTimeSlots/${masterToken.value}/${date}/${totalDurationOfServicesToAppointment.value}`
      )
  ...
  const changeCountServicesAndTotalPriceAddedToAppointment = ({
    action,
    price,
    serviceId,
    duration
    ...
    switch (action) {
      case ServiceActions.ADD: {
        ...
        totalDurationOfServicesToAppointment.value += duration
        listOfServicesAddedToAppointment.value.push(serviceId)
        break
      }
      case ServiceActions.DELETE: {
        countServicesAddedToAppointment.value = Math.max(
          0,
          countServicesAddedToAppointment.value - 1
        )
        totalPriceOfServicesToAppointment.value = Math.max(
          0,
          totalPriceOfServicesToAppointment.value - price
        )
        totalDurationOfServicesToAppointment.value = Math.max(
          0,
          totalDurationOfServicesToAppointment.value - duration
        )

        listOfServicesAddedToAppointment.value = listOfServicesAddedToAppointment.value.filter(
          (serviceIdInList: number) => serviceIdInList !== serviceId
        )
        break
      }
      default: {
  ...
  return {
    ...
    totalDurationOfServicesToAppointment,
    selectedTime,
    description,
